...
var Index, autoprefixer, clone, nib, path, rupture, stylus;
...
module.exports = new (Index = (function() {
  ...
  Index.prototype.compile = function(filepath, source, debug, error, done) {
    return stylus(source).set('filename', filepath).use(nib())["import"]('nib').use(autoprefixer()).use(rupture()).set('linenos', debug === true).render(function(err, css) {
      if (err != null) {
        error(err);
        return done('');
      } else {
